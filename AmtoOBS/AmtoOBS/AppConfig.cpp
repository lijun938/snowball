    ...
    while (std::getline(file, line)) {
        ...
        if (currentSection == "detector") {
            ...
            if (key == "mode") {
                ...
                if (lower == "qcap" || lower == "710" || lower == "sc0710") {
                    config.captureBackend = CaptureBackend::QCAP;
                } else if (lower == "opencv" || lower == "cv" || lower == "cam" || lower == "camera") {
                    config.captureBackend = CaptureBackend::OpenCV;
                } else {
                    errorMessage = "config.ini:" + std::to_string(lineNum) + " invalid capture mode: " + value + " (use qcap or opencv)";
                    return false;
                }
            } else if (key == "opencv_index") {
                if (value.empty()) continue;
                if (!ParseInt(value, config.opencvDeviceIndex)) {
                    errorMessage = "config.ini:" + std::to_string(lineNum) + " invalid opencv_index: " + value;
                    return false;
                }
            } else if (key == "width") {
                if (value.empty()) continue;
...
bool SaveConfigToIniFile(const std::string& iniPath, const AppConfig& config, std::string& errorMessage) {
    ...
    file << "[capture]\n";
    file << "mode=" << (config.captureBackend == CaptureBackend::OpenCV ? "opencv" : "qcap") << "\n";
    file << "opencv_index=" << config.opencvDeviceIndex << "\n";
