...
cv::Mat BuildPreviewPlaceholder(
    ...
    lines.push_back("Waiting for QCAP preview frame");
    lines.push_back(
...
int main(int argc, char** argv) {
    ...
    if (!appConfig.detector.tensorRtRoi.enabled) {
        const QCAPFrameInfo frameInfo = qcapCapture.GetFrameInfo();
        const int srcW = frameInfo.width > 0 ? frameInfo.width : appConfig.capture.width;
        const int srcH = frameInfo.height > 0 ? frameInfo.height : appConfig.capture.height;
        ...
        while (true) {
            ...
            if (!warmupCompleted) {
                ...
                if (warmupElapsedMs >= static_cast<long long>(appConfig.warmupSeconds * 1000.0)) {
                    ...
                    qcapCapture.ResetRuntimeTelemetry();
            ...
            if (elapsedMs >= 1000) {
                std::cout << "[Run] movementTicks=" << totalFrames
                    ...
                    << std::endl;
                PrintRunStatsWindow(runWindowStats, movementController, qcapCapture);
                runWindowStats.Reset();
            }
    ...
    while (!movementTransportSmokeMode) {
        ...
        if (gotFrame) {
...

            {
                ...
                if (!debugDumpQcapFrameWritten && !appConfig.debugDumpQcapFramePath.empty()) {
                    cv::Mat debugFrame;
            ...
            if (shouldUpdateGuiPreview) {
                if (!lastGuiDisplayFrame.empty()) {
                    ...
                    cv::Mat placeholder = BuildPreviewPlaceholder(appConfig, detectorReport, qcapCapture, currentFps);
        ...
        if (!warmupCompleted &&
            appConfig.maxSeconds > 0.0 &&
            wallClockElapsedMs >= static_cast<long long>((appConfig.warmupSeconds + appConfig.maxSeconds) * 1000.0)) {
            if (!startupActiveInputFallbackAttempted) {
                    const QCAPRuntimeStats runtimeStats = qcapCapture.GetRuntimeStats();
